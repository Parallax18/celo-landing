...
const mobileOptions = {
  ...
  return (
    <section id='getStarted' className='md:h-screen p-3 md:py-5 py-12 md:p-5' style={{background: '#1B1B18'}}>
        <div className='w-full md:w-2/3 flex flex-col justify-between relative h-1/2 md:mb-0 mb-64'>
            ...
            <span style={{zIndex: 2}}>
                <SectionHeader title="Start your learning journey now!" isWhite={true}/>
            </span>
        </div>
        <div className='flex md:flex-row flex-col-reverse justify-between items-end'>
            <div className='h-full w-full md:w-1/2 mt-7'>
                <p className='text-white mb-10 md:text-base text-sm'>Voluptatem quos et dolorem nihil alias. Quaerat velit assumenda animi ut et voluptatem sequi omnis. Provident exercitationem expedita quisquam officia ut expedita et. </p>
            ...
            <div className='flex md:flex-row flex-col md:space-y-0 space-y-5'>
                {data.map((item, index) => (
                    <CardComponent key={index} title={item.title} subtitle={item.subtitle} prize={item.prize}variant={item.variant} to={item.to} type="journey"/>
                ))}
            </div>
        </div>
    </section>
