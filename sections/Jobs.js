  ...
  return (
    <section id='jobs' className='px-5 md:py-0 py-16 md:mt-0 mt-10'>
        ...
        <div className='md:flex hidden'>
            {data.map((item, index) => (
                <CardComponent key={index} title={item.title} subtitle={item.subtitle} variant="job" type="job" to='/'/>
            ))}
        </div>
        <div className='md:hidden flex flex-col'>
            {data.map((item, index) => (
                <MobileJobCard key={index} title={item.title} subtitle={item.subtitle} variant="job" type="job" to='/'/>
            ))}
        </div>
